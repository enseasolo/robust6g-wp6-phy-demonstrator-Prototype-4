...
This document accompanies `openapi.yaml` and answers the two questions raised
for ZTSO integration:

1. **What interfaces does the component expose?** — see *§ 2 Interface*.
2. **How will you receive and run it?** — see *§ 3 Delivery & Deployment*.

A short *§ 4 Modelling guidance for the Security Ontology* is included to
help the ZTSO team map the component without round-trips, plus *§ 5 OpenC2
mapping* showing the intended action verbs.
...
contains the result. There is no session or job-handle indirection.
...
### 2.3 Response shape (jamming example)
...
For lab integration: none (HTTP, plain JSON). If required by the ZTSO
deployment, a static bearer-token header (`Authorization: Bearer <token>`)
...
The demonstrator is delivered as a **GitHub repository plus a
...
Suggested mapping for the ZTSO ontology. Refine as needed.
...
`snr_db`, `pj_dbm`, `n_steps` (jamming), `seed` (SKG). These are the knobs
the orchestrator can tune at request time.

---

## 5. OpenC2 mapping

Suggested actuator profile for the ad-hoc OpenC2 actuator the ZTSO team
plans to build.
...
1. **Authentication** — does the ZTSO deployment require mTLS or a bearer
   token in the lab environment, or is plain HTTP within the lab network
   acceptable for v0.1?
2. **Dataset distribution** — the CSI dataset (`data_ULA_skg.npz`,
   ~ 80 MB) ships inside the Docker image. Confirm this is acceptable; if
   not, we can move it to an init-container fetch.
3. **Concurrency** — the SKG endpoint serialises on a global lock (the
   reconciliation worker is not reentrant). Acceptable for a demo. Confirm
   the orchestrator will not issue parallel SKG requests, or we add a
   queue.
4. **Versioning** — API path is `/api/v1`. Breaking changes will bump to
   `/api/v2`; additive changes stay on v1.
